...
namespace Archivarius.Storage
{
    public class ReadOnlyKeyValueStorage : IReadOnlyKeyValueStorage
    {
        ...
        public async Task<TData?> Get<TData>(FilePath path) where TData : class, IDataStruct
        {
            ...
            {
                var success = _deserializer.DeserializeClass<TData>(stream, out var data);
                if (!success)
                {
        ...
        public async Task<TData?> GetStruct<TData>(FilePath path) where TData : struct, IDataStruct
        {
            ...
            await _storage.Read(path, 0, async (stream, _) =>
            {
                var success = _deserializer.DeserializeStruct<TData>(stream, out var data);
                if (!success)
                {
                    throw new Exception("Failed to deserialize data");
                }
        ...
        public async Task<TData?> GetVersionedStruct<TData>(FilePath path) where TData : struct, IVersionedDataStruct
        {
            ...
            await _storage.Read(path, 0, async (stream, _) =>
            {
                var success = _deserializer.DeserializeVersionedStruct<TData>(stream, out var data);
                if (!success)
                {
                    throw new Exception("Failed to deserialize data");
                }
        ...
        public async Task<IReadOnlyCollection<FilePath>> GetElements(DirPath path, bool recursive)
        {
            return await _storage.GetNested(path, recursive);
        }
    }
    ...
    public class KeyValueStorage : ReadOnlyKeyValueStorage, IKeyValueStorage
    {
        ...
        {
            var bytesList = _serializer.SerializeClass(data);
        ...
        {
            var bytesList = _serializer.SerializeStruct(data);
            await _storage.Write(path, bytesList, (dst, bytes) =>
            {
                dst.Write(bytes, 0, bytes.Length);
                return Task.CompletedTask;
        ...
        {
            var bytesList = _serializer.SerializeVersionedStruct(data);
            await _storage.Write(path, bytesList, (dst, bytes) =>
            {
                dst.Write(bytes, 0, bytes.Length);
                return Task.CompletedTask;
